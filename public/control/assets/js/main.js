...
(function($) {
	 ...
	 * -------------------------------------------------------------------------
	 */
	var $wn = $(window), $document = $(document), $body = $('body');
	...
	$(function() {
		...
		$document.on('change', 'input[type="file"]', function(e) {
			e.$el = $(this);
			e.$txt = e.target.files.length === 0 ? 'Choose File'
		 ...
		 * -------------------------------------------------------------------------
		 */
		var $recordsList = $('.records--list'), $recordsListView = $('#recordsListView');
		...
		if ($recordsListView.length) {
			...
			$recordsList.text($recordsList.data('title'));
		}
		...
		$sidebarNav.on('click', '.is-dropdown > a',
				function(e) {
					e.preventDefault();

					var $el = $(this), $es = $el.siblings('ul'), $ep = $el
							.parent(), $ps = $ep.siblings('.open');
		...
		$todoPanel
				.on(
						...
						function(e) {
							...
							var $t = $(this), $input = $t.find('.form-control');
